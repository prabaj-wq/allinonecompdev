...
const ProcessReports = ({ processContext, onClose }) => {
  ...
  const processEntityAmounts = (entries) => {
    ...
    entries.forEach(entry => {
      ...
      } else if (entry.origin === 'other' || entry.card_type === 'other_amounts') {
      ...
      <div key={node.hierarchy_id} className="hierarchy-node">
        <div
          className={`flex items-center py-2 px-3 hover:bg-gray-50 cursor-pointer rounded-lg transition-colors ${
            selectedHierarchy?.hierarchy_id === node.hierarchy_id ? 'bg-blue-50 border-l-4 border-blue-500' : ''
          }`}
        ...
        >
          {node.children && node.children.length > 0 && (
            ...
            >
              {expandedNodes.has(node.hierarchy_id) ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
            </button>
          )}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-blue-100 text-sm">
                Process: {processContext.processName} | Period: {processContext.periodNames?.join(', ')}
              </p>
            </div>
          </div>
          <button
          ...
          <div className="flex-1 overflow-auto">
            {reportData ? (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        ...
                        {reportSettings.roundingFactor > 1 && ` (in ${reportSettings.roundingFactor === 1000 ? 'thousands' : 'millions'})`}
                      </p>
                    </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        {entities.map(entity => (
                          <React.Fragment key={entity.entity_id}>
                            <th
                              colSpan={reportSettings.showICAmounts && reportSettings.showOtherAmounts ? 4 : 2}
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {reportData.sections && Object.values(reportData.sections).map(section => (
                        <React.Fragment key={section.title}>
                          <tr className="bg-blue-50">
                            <td colSpan={100} className="px-4 py-3 text-sm font-bold text-blue-900">
                              {section.title}
                            </td>
                          </tr>
                          {section.accounts && section.accounts.map(account =>
                          ...
                          <tr className="bg-gray-100 font-bold">
                            ...
                            {entities.map(entity => (
                              <React.Fragment key={`total-${entity.entity_id}`}>
                                <td colSpan={reportSettings.showICAmounts && reportSettings.showOtherAmounts ? 4 : 2}
                            ...
                            <td className="px-4 py-3 text-sm text-right bg-gray-200">
                              {formatNumber(section.total || 0, reportSettings.roundingFactor)}
                            </td>
                          </tr>
                        </React.Fragment>
                      ))}
              ...
              <div className="flex flex-col items-center justify-center h-full text-gray-400">
                ...
                <p className="text-sm mt-2">Select a hierarchy and click Generate Report</p>
              </div>
            )}
          </div>
        </div>
