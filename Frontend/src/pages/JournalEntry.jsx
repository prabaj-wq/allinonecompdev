...
import {
  Plus, Save, Trash2, ChevronDown, ChevronRight, Check, X, AlertCircle, CheckCircle, Repeat, Settings
} from 'lucide-react'
...
const JournalEntry = () => {
  ...
  useEffect(() => {
    if (!processId || !scenarioId || !period) {
      alert('Missing required parameters: processId, scenarioId, and period')
      navigate('/process')
    }
  ...
  const addCategory = async () => {
    ...
    try {
      const response = await fetch(`/api/journal-entry/categories?company_name=${selectedCompany}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category_code: newCategoryCode.toUpperCase(),
  ...
  const fetchEntities = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        console.log('Entities fetched:', data.elements)
  ...
  const fetchAccounts = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        console.log('Accounts fetched:', data.elements)
    ...
    try {
      ...
      const batchResponse = await fetch(`/api/journal-entry/batches?company_name=${selectedCompany}`, {
        ...
        body: JSON.stringify({
          ...
          recurring_repeat_days: isRecurring ? recurringDayOfMonth : null
        })
      })
      ...
      <div className="w-80 bg-white dark:bg-gray-800 border-r flex flex-col overflow-hidden">
        <div className="p-4 border-b">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Process: {processId} | Period: {period}
          </p>
        </div>
        ...
        <div className="p-4 border-t space-y-2">
          {!showAddCategory ? (
            <button
              ...
              className="w-full flex items-center justify-center px-4 py-2 border border-dashed border-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700"
            >
            ...
            <div className="space-y-2">
              ...
              <div className="flex space-x-2">
                <button onClick={addCategory} className="flex-1 px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
                  Add
                </button>
                <button onClick={() => setShowAddCategory(false)} className="flex-1 px-3 py-2 border rounded hover:bg-gray-50 text-sm">
                  Cancel
                </button>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 border-b p-6">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Entity: {entityId || 'All'} | Scenario: {scenarioId}
                  </p>
                </div>
                <button
