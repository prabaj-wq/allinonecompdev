...
async def delete_hierarchy_node(
    ...
    try:
        with get_company_connection(company_name) as conn:
            ...
            # Check if node has assigned accounts
            cur.execute("SELECT COUNT(*) FROM axes_accounts WHERE node_id = %s AND company_id = %s", (node_id, company_name))
            account_count = cur.fetchone()[0]
            
            if not cascade and (child_node_count > 0 or account_count > 0):
                raise HTTPException(
                    status_code=400,
                    detail=f"Cannot delete node with {child_node_count} child node(s) and {account_count} assigned account(s). Use cascade=true to delete all."
                )

            if cascade:
                # Unassign all accounts from this node and child nodes
                cur.execute("""
                    UPDATE axes_accounts 
                    SET node_id = NULL 
                    WHERE node_id IN (
                ...
                # Unassign accounts from this node only
                cur.execute("UPDATE axes_accounts SET node_id = NULL WHERE node_id = %s AND company_id = %s", (node_id, company_name))
