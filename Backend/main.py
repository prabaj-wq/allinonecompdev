...
from models.financial_process import (
    FinancialProcess,
    ProcessNode as FPProcessNode,
    ProcessConnection,
    ProcessScenario as FPProcessScenario,
    EntityStructure as FPEntityStructure,
    ConsolidationRule,
    ProcessData as FPProcessData,
    ProcessExecution as FPProcessExecution,
    ProcessJournal as FPProcessJournal,
    FXRate,
    ValidationRule as FPValidationRule,
    ProcessAlert,
    ProcessAuditTrail as FPProcessAuditTrail,
)
...
async def check_page_permissions(request: Request, call_next):
    ...
    if user_id and company_name:
        ...
        try:
            ...
            if user_perms:
                ...
                role_page_perms = (
                    role_permissions if isinstance(role_permissions, dict) else {}
                )
    ...
    if path.startswith("/api/role-management/") and path not in [
        "/api/role-management/login",
        "/api/role-management/user-info",
        "/api/role-management/logout",
    ]:
    ...
    if path.startswith("/api/"):
        try:
            ...
            if user_permissions:
                # Parse permissions
                user_perms = (
                    json.loads(user_permissions["permissions"])
                    if user_permissions["permissions"]
                    else {}
                )
                role_perms = (
                    json.loads(user_permissions["page_permissions"])
                    if user_permissions["page_permissions"]
                    else {}
                )
                ...
                role_page_permissions = (
                    role_perms if isinstance(role_perms, dict) else {}
                )
...
def create_session(
    company_name: str, username: str, user_id: int = None, role: str = "user"
) -> str:
...
def verify_credentials(
    company_name: str, username: str, password: str
) -> Optional[dict]:
    ...
    try:
        ...
        main_cur.execute(
            "SELECT name, code FROM companies WHERE name = %s AND status = 'active'",
            (company_name,),
        )
...
def get_companies_direct(request: Request):
    ...
    try:
        ...
        if authorization and authorization.startswith("Bearer "):
            ...
            try:
                payload = jwt.decode(
                    token, "your-secret-key-change-in-production", algorithms=["HS256"]
                )
        ...
        if user_company:
            ...
            cur.execute(
                "SELECT name, code FROM companies WHERE status = 'active' ORDER BY created_at DESC LIMIT 1"
            )
        ...
        companies = [
            {"name": company[0], "code": company[1]} for company in companies_data
        ]
...
async def role_management_login(request: Request):
    ...
    try:
        ...
        if not all([company_name, username, password]):
            raise HTTPException(
                status_code=400,
                detail="Company name, username, and password are required",
            )
...
def check_if_first_install():
    ...
    try:
        ...
        logger.info(
            f"Found {company_count} active companies, first install: {is_first_install}"
        )
...
def on_startup():
    ...
    if not is_first_install:
        logger.info(
            "Existing companies found, skipping automatic table initialization (handled by company creation)"
        )
    else:
        logger.info(
            "First install detected, skipping database initialization until onboarding is complete"
        )
